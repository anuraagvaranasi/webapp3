...
class Acct:
    ...
    @staticmethod
    def by_filter(filter):
        rows = db.select("SELECT * FROM accts WHERE acct_user LIKE ?", ['%' + filter + '%'])
        accts = []
        for row in rows:
            accts.append(Acct._from_row(row))
        return accts
...
class Xact:
    ...
    def new(acct_id, memo, amount):
        ...
        # Implement method to create new transaction.
        # Returns id for the newly inserted transaction. (provided by db.execute())
        return db.execute("INSERT INTO xacts (xact_acct,xact_amount,xact_memo) VALUES (?,?,?)", [acct_id,amount,memo])


    @staticmethod
    def by_id(xact_id):

        # Implement method to return the transaction for a given id.
    ...
    def by_filter(filter):
        ...
        # Implement method to return list of all transactions where 'xact_memo' matches a LIKE filter.
        # SQL query should be ordered so the most recent transaction (by id) is first i.e. index 0.
        rows = db.select("SELECT * FROM xacts WHERE xact_acct LIKE ? \
        ORDER BY xact_timestamp DESC", ['%' + filter + '%'])
        xacts = []
        for row in rows:
            xacts.append(Xact._from_row(row))
        # Returns list of Xact objects
...
def do_transfer(src, dst, amount, memo):
    ...
    """
    if (src_acct is None) or (dst_acct is None) or (src_acct.user == dst_acct.user):
        return "Transfer Failed - Invalid Accounts."

    if amount <= 0: #don't need to check if its a number or not since its a float
    ...
    sql = [
        ...
        "INSERT INTO xacts (xact_acct,xact_amount,xact_memo) VALUES\
         (" + src + "," + str(amount*-1) + "," + memo +")"
        "INSERT INTO xacts (xact_acct,xact_amount,xact_memo) VALUES\
         (" + dst + "," + str(amount) + "," + memo +")"
        "UPDATE accts SET balance = " + str(src_acct.balance-amount) + " WHERE acct_id = " + src,
        "UPDATE accts SET balance = " + str(dst_acct.balance+amount) + " WHERE acct_id = " + dst
